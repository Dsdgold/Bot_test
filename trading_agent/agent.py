...
class TradingAgent:
    ...
    async def _generate_ai_signal(self, tech_signal: Signal) -> Signal:
        ...
        # use the strategy signal instead (AI is too conservative)
        if ai_signal.side is None and tech_signal.side is not None and tech_signal.confidence >= 25:
            ...
            if up_count >= 2 or down_count >= 2:
                ...
                # Use strategy signal with aggressive params + tighter SL for better RR
                ...
                tech_signal._ai_stop_loss_pct = 0.8
                tech_signal._ai_take_profit_pct = 4.0
                tech_signal._ai_trailing_stop_pct = 0.6
                ...
                tech_signal.reasons = [
                    ...
                    f"Lev:35x SL:0.8% TP:4.0%",
                ]
                logger.info(
                    f"OVERRIDE: AI said WAIT but {down_count}+ TFs DOWN / {up_count}+ TFs UP — "
                    f"using strategy {override_side.value} with aggressive params"
                )
                return tech_signal
    ...
    async def _monitor_position(self, tech_signal: Signal):
        ...
        # Calculate dollar PnL for profit protection
        dollar_pnl = self.position.unrealized_pnl
